...
def log_in(request):
    ...
    if request.method == "GET":
        ...
        return render(
            request, "log_in.html", {"id": "", "type": 1, "src": image_ver[1]}
        )
    ...
    return render(
        request,
        "log_in.html",
        {"error_msg": error_msg, "id": id, "type": type, "src": image_ver[1]},
    )
...
def homepage(request):
    ...
    quarter_xaxis = [
        "{}~{}".format(
            (current_time - timedelta(days=7 * n)).strftime("%m.%d"),
            (current_time - timedelta(days=7 * (n - 1))).strftime("%m.%d"),
        )
        for n in range(12, 0, -1)
    ]
    quarter_sale = {
        quarter_xaxis[i]: {"sale_data": 0, "pure_data": 0}
        for i in range(len(quarter_xaxis))
    }
    # chart 2
    shop_sale = {obj.name: {"sale_data": 0, "pure_data": 0} for obj in shop_query_set}
    # chart 3
    provinces_list = [
        "河北",
        "山西",
    ...
    map_sale = {name: 0 for name in provinces_list}
    ...
    # chart 4
    type_sale = {CHOICE_TYPE[i]: 0 for i in range(1, 6)}

    # chart 5
    weekday_sale = {
        value: {"sale_data": 0, "order_data": 0, "pure_data": 0}
        for _, value in datetime_weekday_convert.items()
    }
    # chart 6
    ...
    for order_obj in order_query_set:
        order_shop, order_goods, order_money, order_time = (
            order_obj.shop_name,
            order_obj.goods_name,
            float(order_obj.cost_money),
        ...
        weekday_sale[datetime_weekday_convert[order_weekday]][
            "pure_data"
        ] -= order_money
        weekday_sale[datetime_weekday_convert[order_weekday]][
            "order_data"
    ...
    if sale_total == 0.0:
        ...
        gauge_rate = round(
            max(0, revenue_rate(sale_total - order_total, sale_total)) * 100, 2
        )
    ...
    bar_one_data = {
        "xaxis": quarter_xaxis,
        "sale_data": [
            round(value["sale_data"], 2) for key, value in quarter_sale.items()
        ],
        "pure_data": [
            round(value["pure_data"], 2) for key, value in quarter_sale.items()
        ],
    }
    # chart 2
    sorted_shop_sale = sorted(
        shop_sale.items(),
        key=lambda x: (x[1]["sale_data"], x[1]["pure_data"]),
        reverse=True,
    )
    valid_sorted_shop_sale = min(len(sorted_shop_sale), 5)
    shop_sale_dict = {
        sorted_shop_sale[valid_sorted_shop_sale - i][0]: sorted_shop_sale[
            valid_sorted_shop_sale - i
        ][1]
        for i in range(valid_sorted_shop_sale)
    }
    bar_hrz_data = {
        "yaxis": list(shop_sale_dict.keys()),
        "sale_data": [
            round(value["sale_data"], 2) for _, value in shop_sale_dict.items()
        ],
        "pure_data": [
            round(value["pure_data"], 2) for _, value in shop_sale_dict.items()
        ],
    }
    # chart 3
    map_data = {
        ...
        "sale_data": [
            {"name": name, "value": round(value, 1)} for name, value in map_sale.items()
        ],
    }
    # chart 4
    donut_data = {
        ...
        "pie_data": [
            {"value": round(value, 2), "name": key} for key, value in type_sale.items()
        ],
    }
    # chart 5
    line_data = {
        "pure_data": [
            round(value["pure_data"], 2) for _, value in weekday_sale.items()
        ],
        "order_data": [
            round(value["order_data"], 2) for _, value in weekday_sale.items()
        ],
        "sale_data": [
            round(value["sale_data"], 2) for _, value in weekday_sale.items()
        ],
    }
...
def shop_list(request):
    ...
    table_body = [
        ...
        [
            str(LIST_MODIFY_BUTTON + LIST_REDIRECT_BUTTON).format(obj.id, obj.id)
...
def shop_add(request):
    ...
    if form.is_valid():
        ...
        name, location, telephone, salesman_name = (
            form.cleaned_data["name"],
            form.cleaned_data["province"] + "-" + form.cleaned_data["city"],
            form.cleaned_data["telephone"],
            form.cleaned_data["salesman_name"],
        )
        ...
        models.shop.objects.create(
            ...
            location_specific=location_spec,
        )
...
def shop_modify(request, nid):
    ...
    if form.is_valid():
        form.save(commit=False)
        name, telephone, salesman_name = (
            form.cleaned_data["name"],
            form.cleaned_data["telephone"],
            form.cleaned_data["salesman_name"],
        )
        location_spec = form.cleaned_data["location_specific"]
        models.shop.objects.filter(id=nid).update(
            name=name,
            telephone=telephone,
            salesman_name=salesman_name,
            location_specific=location_spec,
...
def shop_salesman(request, nid):
    ...
    if nid == 0:
        ...
        table_body = [
            ...
            [
                str(LIST_MODIFY_BUTTON + LIST_DELETE_BUTTON).format(obj.id, obj.id)
                for obj in data_list
            ],
        ]
    else:
...
def shop_salesman_add(request, nid):
    ...
    if form.is_valid():
        ...
        if nid == 0:
            partial_instance.shop_id = models.shop.objects.get(
...
def shop_salesman_modify(request, nid):
    ...
    form = salesmanmodifyform(
        instance=obj, initial={"shop_name": obj.shop_id.name}, data=request.POST
    )
    return render(
        request,
        "_form.html",
...
def calculate_cost_unit(obj):
    ...
    if true_order:
        return round(
            sum([obj.cost_money for obj in true_order])
            / sum([obj.goods_order_num for obj in true_order]),
            2,
        )
    return 0.0
...
def goods_list(request):
    ...
    table_body = [
        ...
        [
            compare_then_arrow(table_unit_sale[i], obj)
            for i, obj in enumerate(data_list)
        ],
        [obj.measurement for obj in data_list],
        [
            (
                SPAN_SUCCESS.format(obj.status)
                if obj.status == "已确认"
                else SPAN_DANGER.format(obj.status)
            )
            for obj in data_list
        ],
        [LIST_MODIFY_BUTTON.format(obj.id) for obj in data_list],
...
def goods_modify(request, nid):
    ...
    if form.is_valid():
        ...
        models.goods.objects.filter(id=nid).update(
            status="已确认",
            type=modified_data["type"],
            unit_sale=modified_data["unit_sale"],
            measurement=modified_data["measurement"],
        )
        return redirect("/goods/list")
    form = goodsform_modify(
        instance=obj, initial={"order_unit": cost_ave}, data=request.POST
    )
    return render(
        request,
        "_form.html",
...
def cost_list(request):
    ...
    table_body = [
        ...
        [
            (
                SPAN_WARNING.format(obj.status)
                if obj.status == "预订购"
                else SPAN_SUCCESS.format(obj.status)
            )
            for obj in data_list
        ],
        [LIST_MODIFY_BUTTON.format(obj.id) for obj in data_list],
...
def cost_modify(request, nid):
    ...
    if form.is_valid():
        ...
        curr_cost_unit, prev_cost_unit = round(curr_cost_unit, 2), round(
            prev_cost_unit, 2
        )
...
def order_add(request):
    ...
    if form.is_valid():
        ...
        remain = models.remain.objects.filter(
            shop_name=shop_name, goods_name=goods_name
        ).values_list("goods_remain", flat=True)
        if not remain:
            models.remain.objects.create(
                shop_name=shop_name,
                goods_name=goods_name,
                goods_remain=int(goods_order_num),
            )
        else:
            models.remain.objects.filter(
                shop_name=shop_name, goods_name=goods_name
            ).update(goods_remain=int(remain[0]) + int(goods_order_num))

        ## 获取obj对象和当前进货单价
        goods_obj = models.goods.objects.filter(name=goods_name)
        cost_obj = models.cost.objects.filter(
            supplier_name=supplier_name, goods_name=goods_name
        )
        ...
        if goods_obj and cost_obj:
            ...
            if current_cost_unit != previous_cost_unit:
                compare = (
                    1 if current_cost_unit < previous_cost_unit else 3
                )  # 与上一次进货单价比较，为1则小于，3则大于
                goods_obj.update(status="未确认")
                cost_obj.update(
                    cost_unit=current_cost_unit, status="已进货", compare=compare
                )
                send_info_to_mail(
                    cost_instance.supplier_name,
                    cost_instance.goods_name,
                    previous_cost_unit,
                    current_cost_unit,
                )
        ## goods_obj 或者 cost_obj 其中一个 不存在，则创建
            ...
            if not goods_obj:
                ...
                models.goods.objects.create(
                    type=1, name=goods_name, unit_sale=sell_unit, measurement="个"
                )
                if cost_obj:
        ...
        if total_len != -1:
            return render(
                ...
                "_upload.html",
                {
                    "form": form,
                    "people_name": request.session.get("name"),
                    "process_result": "<p style='color: green; display: inline'>文件已成功上传，共写入%d条订单记录！</p>"
                    % total_len,
                },
            )
        return render(
            request,
            "_upload.html",
            {"form": form, "people_name": request.session.get("name")},
        )
    else:
        form = file_form()
    return render(
