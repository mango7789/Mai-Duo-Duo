...
class salesmanform(forms.ModelForm):
    ...
    def validate_password(self, password):
        # 密码长度至少为8
        if len(password) < 8:
            raise ValidationError("密码长度至少为8")
        required_classes = {
...
class goodsform_modify(forms.ModelForm):
    ...
    unit_sale = forms.DecimalField(
        max_digits=8, decimal_places=2, label="销售单价"
    )  # 商品销售单价
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["type"].widget = forms.Select(
            choices=mg_models.goods.type_choice
        )  # 添加select的表单形式即可展示choices的文本信息
        for name, field in self.fields.items():
            field.widget.attrs = {"class": "form-control", "placeholder": field.label}
...
class cost_form(forms.ModelForm):
    ...
    def clean_supplier_name(self):
        ...
        # 先把所有的供应商名称提取出来
        supplier_name_set = list(
            mg_models.supplier.objects.values_list("name", flat=True)
        )
    ...
    def clean_goods_name(self):
        ...
        goods_and_supplier = list(
            mg_models.cost.objects.values_list("goods_name", "supplier_name")
        )
...
class order_form(forms.ModelForm):
    ...
    cost_money = forms.DecimalField(
        max_digits=12, decimal_places=2, label="向供应商支付金额"
    )
...
class file_form(forms.Form):
    file = forms.FileField(
        label="支持.csv,.xls和.xlsx格式，文件大小不超过20MB", required=True
    )
