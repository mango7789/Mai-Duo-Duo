...
class supplier(models.Model):
    ...
    email = models.CharField(
        max_length=30, null=True, unique=True, verbose_name="该邮箱"
    )
...
class shop(models.Model):
    ...
    status = models.SmallIntegerField(
        verbose_name="营业状态", choices=status_choices, default=1
    )
...
class goods(models.Model):  # 商品信息表，注意，包含销售单价信息
    ...
    type = models.SmallIntegerField(
        verbose_name="商品类别", choices=type_choice, default=1
    )  # 商品类别，枚举
...
class cost(models.Model):  # 进货单价表
    ...
    goods_name = models.CharField(
        default=None, max_length=20, db_index=True
    )  # 外键约束放到钩子函数进行处理
