...
def random_image(images_dict: dict) -> list:
    ...
    authen_code, img_path = (
        images_dict[str(images_num)][0],
        "/static/captcha/" + images_dict[str(images_num)][1],
    )
...
def database_dump(table: Model, file_path: str, Header: list[str]) -> None:
    ...
    data_frame.to_excel(
        file_path, index=False, header=Header if len(Header) > 0 else True
    )
    return


def send_info_to_mail(
    supplier: str, goods: str, prev: float, curr: float, type: str = "order"
) -> None:
    # type = ["order", "cost"]
    text = (
        "<b>"
...
def read_datafile(file_path: str):
    ...
    error_msg_list.append(
        """<p style="color: purple; display:inline; font-weight: bold; font-size: 12px">字段要求：供应商名称（存在）、商品名称（长度不超过20）、商品订量（正整数）、金额（正数）、门店名称（存在）</p>"""
    )
    for index, row in df.iterrows():
        ...
        if (
            not isinstance(cost_money, int) and not isinstance(cost_money, float)
        ) or cost_money <= 0:
            correct = False
            flg = True
            field.append("金额")
    ...
    for index, row in df.iterrows():
        ...
        if not remain:
            mg_models.remain.objects.create(
                shop_name=shop_name,
        ...
        cost_obj = mg_models.cost.objects.filter(
        ...
        ## 保存进货记录
        mg_models.order_record.objects.create(
            supplier_name=supplier_name,
            goods_name=goods_name,
