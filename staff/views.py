...
def staff_shop_list(request):
    ...
    if value:
        ...
        remain_list = (
            mg_models.remain.objects.filter(goods_name__icontains=value)
            .filter(goods_remain__gt=0)
            .values_list("shop_name", "goods_remain", "goods_name")
        )
        data_list = []
        for i in range(len(remain_list)):
            shop_name, remain_num, goods_name = (
                remain_list[i][0],
                remain_list[i][1],
                remain_list[i][2],
            )
            shop_obj = mg_models.shop.objects.filter(name=shop_name).first()
            goods_obj = mg_models.goods.objects.filter(
                name=goods_name, status="已确认"
            ).first()
        ...
        table_body = [
            ...
            [
                "<b>" + obj.name + "</b>" if obj.name == shop_name else obj.name
                for obj in data_list
            ],
            [
                "".join(obj.location.split("-")) + obj.location_specific
                for obj in data_list
            ],
...
def staff_goods_list(request):
    ...
    if value:  # 返回本店中名称包含该字段的商品余量信息
        remain_list = mg_models.remain.objects.filter(
            shop_name=shop_name, goods_name__icontains=value
        ).filter(goods_remain__gt=0)
    else:  # 如果没有传参数q，就全部返回所有商品的余量信息
        remain_list = mg_models.remain.objects.filter(shop_name=shop_name).filter(
            goods_remain__gt=0
        )
...
class saleform(forms.ModelForm):
    goods_name = forms.CharField(
        max_length=20, label="商品名称", required=False, disabled=True
    )
...
def staff_sale_receipt_add(request, nid):
    ...
    shop_name = mg_models.shop.objects.filter(id=shop_id).values_list(
        "name", flat=True
    )[0]
    remain = mg_models.remain.objects.filter(
        shop_name=shop_name, goods_name=goods_obj.name
    ).first()
    initial_data = {
        "goods_name": goods_obj.name,
        "goods_type": choice_type[goods_obj.type],
        "measurement": goods_obj.measurement,
        "remain": remain.goods_remain,
    ...
    if form.is_valid():
        ...
        sale_price = mg_models.goods.objects.filter(name=goods_obj.name).values_list(
            "unit_sale", flat=True
        )[0]
        remain = (
            mg_models.remain.objects.filter(
                shop_name=shop_name, goods_name=goods_obj.name
            )
        ...
        mg_models.remain.objects.filter(
            shop_name=shop_name, goods_name=goods_obj.name
        ).update(goods_remain=int(remain) - int(goods_sale_num))
        mg_models.sale_record.objects.create(
            shop_name=shop_name,
            goods_name=goods_obj.name,
            goods_sale_num=goods_sale_num,
            turnover=turnover,
            sale_man_id=request.session.get("id"),
        )
...
def staff_sale_list(request):
    ...
    value = request.GET.get("q", "")
    data_list = mg_models.sale_record.objects.filter(
        shop_name=name, goods_name__icontains=value
    ).order_by("-sale_time")
